...
describe("OurNFTContract", function () {
  ...
  beforeEach(async () => {
    ...
    hardhatOurNFTContract = await ourNFTContract.deploy(
      1,
  ...
  it("Coordinator should fulfill Random Number request", async () => {
    ...
    let [reqId, invoker] = events.filter(
  ...
  it("Contract should receive Random Numbers", async () => {
    ...
    expect(await hardhatOurNFTContract.getCharacter(0)).to.include(
